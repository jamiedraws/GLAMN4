...
{
    public class N3PAYPageHandler : PageHandler
    {
        #region " Overrides... "
        ...
        {
            string[] upgrade = { "N3PAY", "N1PAY" };
 
            var deluxe = Order.Items["N4KIT"].Quantity;

            foreach (string product in upgrade)
            {
                if (deluxe == 1)
                {
                    if (Order.Items[product].Quantity > 0)
                    {
                        OrderManager.SetProductQuantity(product, 0);

                        OrderManager.UpgradeProduct("N4KIT", product);
                    }
                }
                ...
                else if (deluxe > 1)
                {
                    if (Order.Items[product].Quantity > 0)
                    {
                        var remainder = deluxe - Order.Items[product].Quantity;

                        OrderManager.SetProductQuantity("N4KIT", remainder);
                    }
